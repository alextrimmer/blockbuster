 ...
 */
public class Data {
  static HashMap<Integer, Video> hashed = new HashMap<Integer, Video>();
  private Data() {}
  /**
  ...
  static public Video newVideo(String title, int year, String director) {
    ...
    if (hashed.containsValue(v)) {
      return hashed.get(v.hashCode());
    } else {
      hashed.put(v.hashCode(),v);
      return hashed.get(v.hashCode());
    }
  }
  ...
  static public RerunnableCommand newUndoCmd(Inventory inventory) {
    ...
    return new RerunnableCommand() {
      @Override
      public boolean run() {
        InventorySet i = (InventorySet) inventory;
        boolean test = i.getHistory().getUndo().run();
        return test;
      }
    };
  ...
  static public RerunnableCommand newRedoCmd(Inventory inventory) {
    ...
    return new RerunnableCommand() {
      ...
      public boolean run() {
        InventorySet i = (InventorySet) inventory;
        boolean test = i.getHistory().getRedo().run();
        return test;
      }
    };
  }
