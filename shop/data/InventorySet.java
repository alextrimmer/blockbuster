...
final class InventorySet implements Inventory {
  ...
  Record checkOut(Video video) {
    RecordObj newR = (RecordObj) _data.get(video);
    if (newR == null || newR.numOut == newR.numOwned) {
      throw new IllegalArgumentException();
    }
    _data.put(video, new RecordObj(video,
            newR.numOwned,
            newR.numOut+1,
            newR.numRentals+1));
    return newR;
